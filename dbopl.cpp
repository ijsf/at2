...
inline void Channel::GeneratePercussion( Chip* chip, Bit32s* output ) {
	...
	sample <<= 1;
	if ( opl3Mode ) {
		output[0] += sample & maskLeft;
		output[1] += sample & maskRight;
	} else {
		output[0] += sample;
	}
}
...
extern "C" Bit32u DBOPL_WriteAddr( Bit32u port, Bit8u val ) {
	return chip.WriteAddr( port, val );

}

extern "C" void DBOPL_WriteReg( Bit32u addr, Bit8u val ) {
	chip.WriteReg( addr, val );
}

extern "C" int DBOPL_Generate( Bitu total, Bit32s* output ) {
	if ( !chip.opl3Active ) {
		chip.GenerateBlock2( total, output );
		return 1;
	} else {
		chip.GenerateBlock3( total, output );
		return 2;
	}
}

extern "C" void DBOPL_Init( Bitu rate ) {
	InitTables();
	chip.Setup( rate );
}
