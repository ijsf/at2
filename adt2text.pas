...
const
  ...
  ascii_line_23 = 'ณ `special thanks:`      HOMEPAGE   www.adlibtracker.net ณ';
  ascii_line_24 = 'ณ ~encore~               EMAiL  subz3ro.alrair@gmail.com ณ';
  ascii_line_25 = 'ณ ~insane/Altair~                      flo@windfisch.org ณ';
  ascii_line_26 = 'ภ-ฤฤฤฤฤฤฤฤฤฤ--๙๚    ๚๙-ฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤ-ู';
...
const
  ...
  help_data: array[1..LINES] of String[128] = (
    ...
    'รฤฤฤฤฤฤฤฤฤฤฤฤฤฤยฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤด',
    'ณ ~^Left,^Right~ ณ Select group of tracks for recording                ณ',
    'ณ ~[Alt] Q~      ณ Quick reset last selected group of tracks           ณ',   
    'ณ ~Enter~        ณ Start recording from current position ~(*)~           ณ',
    'ณ ~Space~        ณ `Toggle` using custom instrument for all tracks ฟ     ณ@@spec_attr:01@@',
    'ณ ~[Alt] Space~  ณ `Toggle` using present instruments in tracks    ร (*) ณ@@spec_attr:02@@',
    'ณ ~MBoard keys~  ณ Write notes to corresponding tracks           ณ     ณ',
    'ณ ~F8,F9~        ณ Toggle pattern repeat OFF/ON                  ู     ณ',
    'ณ ~Backspace~    ณ Clear note/instrument sequence in tracks            ณ',
    'ณ ~Up,Down~      ณ Rewind/Fast-Forward while recording                 ณ',
    'ณ ~[Shift] F6~   ณ Continue in Debug mode from position at cursor      ณ',
    'ณ ~F7~           ณ Stop recording and reset starting position;         ณ',
    'ณ              ณ current group of tracks can be modified             ณ',
    'ณ ~[Alt] 1..9,0~ ณ Toggle corresponding track ON/OFF                   ณ',
    'ณ ~[Alt] R~      ณ Reset flags on all tracks                           ณ',
    'ณ ~*~            ณ Reverse ON/OFF on all tracks                        ณ',
    'รฤฤฤฤฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤด',
    'ณ In case you need non-continuos track selection, you can choose     ณ',
    'ณ from already selected group a subset of tracks where notes will be ณ',
    'ณ written by manipulating track ON/OFF flags.                        ณ',
    ...
    'ณ Effect commands are processed with following priority:           ณ',
    'ณ   1) Set instrument volume (~Cxx~),                                ณ',
    'ณ      Force instrument volume (~=xx~)                               ณ',
    'ณ   2) Set modulator volume (~9xx~)                                  ณ',
    'ณ   3) Set carrier volume (~Ixx~)                                    ณ',
    'ณ   4) Set global volume (~%xx~)                                     ณ',
    'ณ If effect command with higher priority has been processed,       ณ',
    'ณ all remaining effect commands with lower priority are skipped.   ณ',
...
{$ENDIF}
    ...
    '    `2`x SET ATTACK RATE   ฟ        x=attack_rate           [0-F]  ~I~',
    '    `3`x SET DECAY RATE    ๖ MOD.   x=decay_rate            [0-F]  ~I~',
    '    `4`x SET SUSTAiN LEVEL ๘        x=sustain_level         [0-F]  ~I~',
    '    `5`x SET RELEASE RATE  ู        x=release_rate          [0-F]  ~I~',
    '    `6`x SET ATTACK RATE   ฟ        x=attack_rate           [0-F]  ~I~',
    '    `7`x SET DECAY RATE    ๖ CAR.   x=decay_rate            [0-F]  ~I~',
    '    `8`x SET SUSTAiN LEVEL ๘        x=sustain_level         [0-F]  ~I~',
    '    `9`x SET RELEASE RATE  ู        x=release_rate          [0-F]  ~I~',
    ...
    '    `1`x SET MULTiPLiER ฟ           x=multiplier            [0-F]  ~I~',
    '    `2`x SET KSL        ๖           x=scaling_level         [0-3]  ~I~',
    '    `3`x SET TREMOLO    ๖ MOD.      x=off/on                [0-1]  ~I~',
    '    `4`x SET ViBRATO    ๘           x=off/on                [0-1]  ~I~',
    '    `5`x SET KSR        ๖           x=off/on                [0-1]  ~I~',
    '    `6`x SET SUSTAiN    ู           x=off/on                [0-1]  ~I~',
    '    `7`x SET MULTiPLiER ฟ           x=multiplier            [0-F]  ~I~',
    '    `8`x SET KSL        ๖           x=scaling_level         [0-3]  ~I~',
    '    `9`x SET TREMOLO    ๖ CAR.      x=off/on                [0-1]  ~I~',
    '    `A`x SET ViBRATO    ๘           x=off/on                [0-1]  ~I~',
    '    `B`x SET KSR        ๖           x=off/on                [0-1]  ~I~',
    '    `C`x SET SUSTAiN    ู           x=off/on                [0-1]  ~I~',
    '',
    '@topic:effects_page6',
    ...
    '`@@`xx ฤฤ SET CUSTOM SPEED TABLE     xx=parameter            [0-FF]',
    '',
    ...
    '',
    '            ฺฤฤฤฤฤฤฤฤฤฤฤฤยฤฤฤฤฤฤยฤฤฤฤฤฤฤฤยฤฤฤฤฤฤฤฤฟ',
    '            ณ PARAMETER  ณ `SiZE` ณ `MAX.`   ณ `FACTOR` ณ',
    '            ฦออออออออออออุออออออุออออออออุออออออออต',
    '            ณ `[01]`..`[EF]` ณ 32   ณ ~01~..~EF~ ณ '#7'1     ณ',
    '            ณ `[F0]`..`[F3]` ณ ~32~   ณ FF     ณ ~'#7'1~..~'#7'4~ ณ',
    '            ณ `[F4]`..`[F7]` ณ ~64~   ณ FF     ณ ~'#7'1~..~'#7'4~ ณ',
    '            ณ `[F8]`..`[FB]` ณ ~128~  ณ FF     ณ ~'#7'1~..~'#7'4~ ณ',
    '            ณ `[FC]`..`[FF]` ณ ~256~  ณ FF     ณ ~'#7'1~..~'#7'4~ ณ',
    '            ภฤฤฤฤฤฤฤฤฤฤฤฤมฤฤฤฤฤฤมฤฤฤฤฤฤฤฤมฤฤฤฤฤฤฤฤู',
    '',
    '`FURTHER NOTES:`',
    '',
...
begin
  For temp := 0 to 18 do
    adt2_title[temp] := RotStrL('/ดDLiB TR/ดCK3R ][', ' [SDL] VER.'+at2ver+' ',temp);

  adt2_title[18] := ' [SDL] VER.'+at2ver+' ';

  For temp := 19 to 36 do
     adt2_title[temp] := RotStrL(' [SDL] VER.'+at2ver+' ', '/ดDLiB TR/ดCK3R ][',temp-18);

  WriteLn;
  WriteLn('/ดDLiB TR/ดCK3R ][ SDL');
