...
begin
  ...
  temp := InputStr(temp,x+1+MenuLib2_mn_environment.edit_pos,y+k,
...
begin { MenuLib2_Menu }
  ...
  For i := 1 to count do
    begin
      ...
      If NOT MenuLib2_mn_setting.reverse_use then mbuf[i].use := mbuf[i].key <> '~'
  ...
  For i := 1 to count do
    If max < CStrLen(pstr(i))+MenuLib2_mn_environment.descr_len then
      max := CStrLen(pstr(i))+MenuLib2_mn_environment.descr_len;
