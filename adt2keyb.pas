    ...
    begin
      ...
      If (SDL_PeepEvents(event,1,SDL_PEEKEVENT,SDL_KEYDOWNMASK) > 0) then
        ...
        If (event.key.keysym.sym >= SDLK_NUMLOCK) then
          begin
            SDL_PeepEvents(event,1,SDL_GETEVENT,SDL_KEYDOWNMASK); // remove event from queue
            CONTINUE;
          end
...
function shift_pressed: Boolean;
begin
  shift_pressed := scankey($2a) or { L-SHIFT } scankey($36); { R-SHIFT }
end;

function left_shift_pressed: Boolean;
begin
  left_shift_pressed := scankey($2a);
end;

function right_shift_pressed: Boolean;
begin
  right_shift_pressed := scankey($36);
end;

function both_shifts_pressed: Boolean;
begin
  both_shifts_pressed := scankey($2a) and scankey($36);
end;

function alt_pressed: Boolean;
begin
  alt_pressed := scankey(SC_LALT) or scankey(SC_RALT); { ALT }
end;

function ctrl_pressed: Boolean;
begin
  ctrl_pressed := scankey(SC_LCTRL) or scankey(SC_RCTRL); { CTRL }
end;
